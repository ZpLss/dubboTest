...
public class MessageConfig {
    ...
    protected static Logger logger = LoggerFactory.getLogger(MessageConfig.class);
...
//			URL classPtah = ParamConfig.class.getResource("/");
            ...
            File classpathFile = new File(classPtah.getPath() + File.separator + "message");
            File[] files = classpathFile.listFiles();
            String[] filesPath = new String[files.length];
            int index = 0;

            //配置文件路径头
            String head = "classpath:/message/";
            for (File file : files) {
                //是properties配置文件
    ...
    public static String getErrorMsg(String key, String... errorMsgParams) {
